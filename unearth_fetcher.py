"""
A Fetcher() for unearth to take advantage of http2 pipelining in httpx from a
number of PackageFinder() worker threads.
"""
...
class SharedAsyncPyPIClient:
    ...
    def __init__(
        ...
        *,
        ...
        http_cache_db_path: str | Path | None = None,
        **kwargs: Any,
    ) -> None:
        ...
        self._http_cache_db_path = http_cache_db_path
    ...
    async def _make_client(self) -> httpx.AsyncClient:
        ...
        mounts = dict(client_kwargs.get("mounts", {}))
        if self._http_cache_db_path is not None:
            storage = AsyncSqliteStorage(database_path=self._http_cache_db_path)
            mounts["http://"] = AsyncCacheTransport(
                httpx.AsyncHTTPTransport(),
                storage=storage,
            )
            mounts["https://"] = AsyncCacheTransport(
                httpx.AsyncHTTPTransport(),
                storage=storage,
            )
    ...
    def get(
        ...
        return cast(
            Response,
            self._run_coroutine(self._client.get(url, headers=headers, auth=self.auth)),
        )
    ...
    def head(
        ...
        return cast(
            Response,
            self._run_coroutine(self._client.head(url, headers=headers, auth=self.auth)),
        )
